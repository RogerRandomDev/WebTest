...
function keyPressed(keyid) {
    ...
    if(keyid.key==" " && rapidjump){
        cx += 512*Math.cos((faceAngle+90)*deg2rad)+512*Math.sin((faceAngle+90)*deg2rad)
        cy += 512*Math.sin((faceAngle+90)*deg2rad)-512*Math.cos((faceAngle+90)*deg2rad)
        rapidjump=false
        window.setTimeout(function(){rapidjump=true},250)
    }
}
